## Koyeb 自动登录保活脚本
这是一个用于自动登录 Koyeb 网站以保持账户活跃的脚本，配合 GitHub Actions 实现自动定时执行。

注册地址：https://app.koyeb.com


### 功能特点
- 🔐 自动登录 Koyeb 账户(单账户或多账户)
- 👥 支持多账户批量处理
- ⏰ 每4天自动执行一次
- 📱 执行结果可通过 Telegram 通知
...
- `ACCOUNTS`  Koyeb账户(必填)，格式(单账号)：email:password   格式(多账号)：email1:password1,email2:password2 

- telegram为可选环境变量,不需要通知可不填写
- `BOT_TOKEN` 	Telegram机器人Token(https://t.me/BotFather)获取
- `CHAT_ID`     Telegram 聊天ID
	    
4. GitHub Actions 初始手动执行检查是否有配置错误，脚本会自动每4天执行一次,可手动执行
